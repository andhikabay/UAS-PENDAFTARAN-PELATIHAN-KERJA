<x-app-layout class="bg-[#EAF6FF]">
  <x-slot name="header"><h2 class="text-xl font-semibold text-[#2F2F2F]">Tambah Peserta</h2></x-slot>
      ...
      <form action="{{ route('peserta.store') }}" method="POST" class="space-y-6">
        ...
        @foreach(['nama','email','no_hp'] as $f)
          <div>
            ...
            <input type="{{ $f=='email'?'email':'text' }}" name="{{ $f }}" value="{{ old($f) }}" class="mt-1 w-full border-gray-300 rounded-md shadow-inner focus:ring-[#74C0FC] focus:border-[#74C0FC]" required>
          </div>
        @endforeach
        <div>
          <label class="block text-gray-700 font-medium">Alamat</label>
          <textarea name="alamat" rows="3" class="mt-1 w-full border-gray-300 rounded-md shadow-inner focus:ring-[#74C0FC] focus:border-[#74C0FC]" required>{{ old('alamat') }}</textarea>
        </div>
        <div class="flex space-x-4 pt-4">
          <button type="submit" class="px-6 py-2 bg-[#74C0FC] text-white rounded-full shadow hover:bg-[#A5D8FF] transition">Simpan</button>
